...
Задача 3 лабораторной работы №2
"""
...
def solve_lmi_problem(A, B):
    """Решение задачи LMI для синтеза регулятора (упрощенная версия)"""
    print("\n" + "=" * 60)
    print("РЕШЕНИЕ ЗАДАЧИ LMI (УПРОЩЕННАЯ ВЕРСИЯ)")
    print("=" * 60)
    
    # Для экспоненциальной устойчивости степени α = 2
    # Используем модифицированную систему: A' = A + αI
    alpha = 2
    A_mod = A + alpha * np.eye(2)
    
    print(f"Модифицированная матрица A' = A + αI = \n{A_mod}")
    ...
    P = solve_continuous_are(A_mod, B, Q, R)
    K = R**(-1) * B.T @ P
    
    print(f"\nРешение через Риккати:")
    print(f"P = \n{P}")
    print(f"K = R^(-1)B^T P = \n{K}")
    ...
    print(f"λ = {eigenvals}")
    print(f"Все собственные значения имеют вещественную часть < -2: {np.all(np.real(eigenvals) < -2)}")
...
def main():
    ...
    # Решение через LMI (упрощенная версия)
