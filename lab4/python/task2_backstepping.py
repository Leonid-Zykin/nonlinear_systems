...
Задача 2 лабораторной работы №4
Система: ẋ₁ = x₂ - x₁³, ẋ₂ = x₁ + u
"""
...
def backstepping_synthesis_task2():
    """Синтез регулятора методом бэкстеппинга для системы 2"""
    print("=" * 80)
    print("СИНТЕЗ РЕГУЛЯТОРА МЕТОДОМ БЭКСТЕППИНГА - ЗАДАЧА 2")
    print("=" * 80)
    ...
    # Рассматриваем первую подсистему: ẋ₁ = x₂ - x₁³
    ...
    V1_dot_sub = x1*(alpha1 - x1**3)  # Подставили α₁
    V1_dot_sub = simplify(V1_dot_sub)
    print(f"\nṼ₁ = x₁(α₁ - x₁³) = {V1_dot_sub}")
    
    # С учетом ошибки
    V1_dot = x1*(alpha1 - x1**3) + x1*z2
    ...
    # ẋ₁ = x₂ - x₁³
    alpha1_dot = alpha1_diff * (x2 - x1**3)
    alpha1_dot = simplify(alpha1_dot)
    print(f"α̇₁ = (∂α₁/∂x₁) · ẋ₁ = {alpha1_dot}")
    
    # ż₂ = ẋ₂ - α̇₁ = x₁ + u - α̇₁
    z2_dot = x1 + symbols('u') - alpha1_dot
...
def simulate_system2(u_func, c1, c2, alpha1_func):
    """Моделирование системы 2"""
    print("\n" + "=" * 60)
    print("МОДЕЛИРОВАНИЕ СИСТЕМЫ 2")
    print("=" * 60)
    ...
    def u_control(x1_val, x2_val):
        # Вычисляем α₁
        alpha1_val = -c1*x1_val + x1_val**3
        
        # Вычисляем z₂
        z2_val = x2_val - alpha1_val
        
        # Вычисляем α̇₁
        alpha1_diff_val = -c1 + 3*x1_val**2
        x1_dot_val = x2_val - x1_val**3
        alpha1_dot_val = alpha1_diff_val * x1_dot_val
        
        # Вычисляем u
        u_val = -c2*z2_val - 2*x1_val + alpha1_dot_val
        return u_val
    ...
    fig.suptitle('Моделирование системы 2 с регулятором бэкстеппинга', fontsize=14)
    ...
    print("Графики сохранены в images/task2/backstepping_system2.png")
...
def main():
    ...
    # Синтез регулятора
    u_final, c1, c2, alpha1 = backstepping_synthesis_task2()
    
    # Моделирование
    simulate_system2(u_final, c1, c2, alpha1)
