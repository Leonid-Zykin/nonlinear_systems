#!/usr/bin/env python3
"""
Улучшенная версия графиков с явными различиями и сравнением с неуправляемыми системами
"""
...
x0_1 = [2.5, 1.8]
sol1_ctrl = solve_ivp(sys1_controlled, (0, 7), x0_1, t_eval=np.linspace(0, 7, 700), rtol=1e-4)
sol1_unctrl = solve_ivp(sys1_uncontrolled, (0, 7), x0_1, t_eval=np.linspace(0, 7, 700), rtol=1e-4, 
                         max_step=0.05)

fig1, axes1 = plt.subplots(2, 2, figsize=(15, 10))
fig1.suptitle('Система 1: ẋ₁ = x₂ + sin x₁ + x₁², ẋ₂ = x₁² + (2 + sin x₁)u', fontsize=14, fontweight='bold')

# Состояния с сравнением
...
axes1[0,0].plot(sol1_unctrl.t, sol1_unctrl.y[0], 'b--', lw=1.5, alpha=0.6, label='x₁(t), неуправляемая')
axes1[0,0].plot(sol1_unctrl.t, sol1_unctrl.y[1], 'r--', lw=1.5, alpha=0.6, label='x₂(t), неуправляемая')
...
u1 = [u_task1(sol1_ctrl.y[0,i], sol1_ctrl.y[1,i]) for i in range(0, len(sol1_ctrl.t), 7)]
axes1[0,1].plot(sol1_ctrl.t[::7], u1, 'g-', lw=2.5)
...
axes1[1,0].plot(sol1_unctrl.y[0], sol1_unctrl.y[1], 'r--', lw=2, alpha=0.6, label='Неуправляемая')
...
x0_2 = [1.8, -1.2]  # Другие начальные условия
sol2_ctrl = solve_ivp(sys2_controlled, (0, 7), x0_2, t_eval=np.linspace(0, 7, 700), rtol=1e-4)
sol2_unctrl = solve_ivp(sys2_uncontrolled, (0, 7), x0_2, t_eval=np.linspace(0, 7, 700), rtol=1e-4, 
                         max_step=0.05)
...
axes2[0,0].plot(sol2_unctrl.t, sol2_unctrl.y[0], 'b--', lw=1.5, alpha=0.6, label='x₁(t), неуправляемая')
axes2[0,0].plot(sol2_unctrl.t, sol2_unctrl.y[1], 'r--', lw=1.5, alpha=0.6, label='x₂(t), неуправляемая')
...
u2 = [u_task2(sol2_ctrl.y[0,i], sol2_ctrl.y[1,i]) for i in range(0, len(sol2_ctrl.t), 7)]
axes2[0,1].plot(sol2_ctrl.t[::7], u2, 'g-', lw=2.5)
...
axes2[1,0].plot(sol2_unctrl.y[0], sol2_unctrl.y[1], 'r--', lw=2, alpha=0.6, label='Неуправляемая')
...
print(f"  - Неуправляемая: финальная норма = {norm1_unctrl[-1]:.6f}")
print(f"  - Характеристика: sin x₁ создает периодические компоненты в начальной фазе")

print(f"\nСистема 2 (x₀ = {x0_2}):")
print(f"  - Управляемая: финальная норма = {norm2[-1]:.6f}")
print(f"  - Неуправляемая: финальная норма = {norm2_unctrl[-1]:.6f}")
print(f"  - Характеристика: кубическая нелинейность x₁³ создает более резкий профиль")

print("\n✓ Теперь различия четко видны:")
print("  - Разные начальные условия: [2.5, 1.8] vs [1.8, -1.2]")
print("  - Разные траектории сходимости")
print("  - Сравнение с неуправляемыми системами показывает эффективность регуляторов")
print("  - Система 1: влияние sin x₁ видно в начальной динамике")
print("  - Система 2: кубическая нелинейность x₁³ создает другой характер сходимости")
